...
function socks5Connect(proxy, target, timeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    ...
    socket.setTimeout(timeoutMs, () => fail(new Error(`socks5 timeout talking to ${proxy.host}:${proxy.port}`)));
  ...
  return new Promise((resolve, reject) => {
    ...
    req.once('connect', (res, socket) => {
      if (res.statusCode !== 200) {
        socket.destroy();
        return reject(new Error(`proxy ${proxyKey(proxy)} refused CONNECT (HTTP ${res.statusCode})`));
      }
      stats.tunnels++;
      socket.setNoDelay(true);
      resolve(socket);
    });
    req.once('timeout', () => req.destroy(new Error(`proxy CONNECT timeout (${proxyKey(proxy)})`)));
    req.once('error', e => reject(new Error(`proxy ${proxyKey(proxy)}: ${e.message}`)));
    req.end();
  });
}
...
function route() {
  ...
  if (config.mode === 'list') {
    ...
    const proxy = config.list[rr++ % config.list.length];
    return { proxy, key: proxyKey(proxy) };
  }
  return null;
}
 ...
 * Called once per outbound check, so it is also where rotation is counted.
 */
...
function setConfig(patch = {}) {
  ...
  if (patch.torSocks !== undefined) {
    const text = String(patch.torSocks || '').trim() || 'socks5://127.0.0.1:9050';
...
module.exports = {
  setConfig, status, describe, initFromEnv, agentFor, reset,
  torNewNym, playwrightProxies, noteExit, parseProxy,
