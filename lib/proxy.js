...
async function checkRoute(timeoutMs = 4000) {
  ...
  if (config.mode === 'tor' && bad.length) {
    ...
    if (found.ok) {
      ...
      hint = `no Tor SOCKS port answered (${ports.join(', ')} probed) — press “▶ start Tor” to run the local daemon, or set the route to direct`;
    } else {
      hint = 'no Tor daemon is running and no tor binary is installed on this host — install Tor (Debian/Ubuntu: apt-get install tor · macOS: brew install tor) and press “▶ start Tor”, or use a proxy list / set the route to direct';
    }
  }
