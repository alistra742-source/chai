'use strict';
/*
 * SNIPR browser swarm — SIX real headless Chrome browsers sniping in parallel,
 * each with a live screenshot feed ("live cam") streamed to the dashboard.
 *
 ...
 *
 * Requires on the host:  npm i playwright  &&  npx playwright install chromium
 * (auto-detected; if missing the server falls back to HYDRA mode — 6 worker
 * threads inside the user's own browser.)
 */
...
class Swarm {
  ...
  async start(run, count = 6) {
    ...
    if (!pw) throw new Error('SWARM_UNAVAILABLE');
    const exe = (() => { try { return pw.chromium.executablePath(); } catch (_) { return null; } })();
    this.browser = await pw.chromium.launch({
      ...
      executablePath: (exe && exe.includes('does-not-exist')) ? undefined : exe || undefined,
    });
...
module.exports = { Swarm, bind, detectPlaywright, isPremiumAlias, UA_LIST, swarm: new Swarm() };
