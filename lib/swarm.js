...
/* Every browser being rejected (401/403/Cloudflare) is not bad luck — stop the
 * run instead of marking the whole wordlist as errors on a flagged exit. */
...
class Swarm {
    ...
    const worker = async (i) => {
      ...
      while (!this.abort && !run.abort) {
        ...
        try {
          ...
          res = { status: 'error', via: 'swarm', kind: e && e.kind, note: String(e.message || e).slice(0, 120) };
        }
        if (res.kind === 'blocked') {
          this.blockedFails = (this.blockedFails || 0) + 1;
          if (this.blockedFails >= BLOCKED_BEFORE_ABORT && !run.abort) {
            run.error = `every browser is being blocked — stopped after ${this.blockedFails} straight rejects (${res.note})`;
            run.abort = true;
            this.abort = true;
          }
  ...
  async checkWithBrowser(page, w, platform, name, run) {
    ...
    if (platform === 'gunslol') {
      // A rejected exit answers with an error page that still has a <title>, so
      // without this it falls through and every name is scored as "taken".
      if (http === 401 || http === 403) {
        const err = new Error(`guns.lol rejected this exit (HTTP ${http}) — needs a cleaner/residential IP`);
        err.kind = 'blocked';
        throw err;
      }
      ...
      if (page_info.title.includes('just a moment') || page_info.title.includes('attention required')) {
        const err = new Error('cloudflare challenge — slow down or use residential IP');
        err.kind = 'blocked';
        throw err;
      }
