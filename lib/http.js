 ...
 *     actually gets a new exit to retry from). A `401` is retried the same way
 *     only when the route can really rotate — it means the target rejected that
 *     exit, and retrying it from the same IP would just add rejected requests.
 *
 *  4. COOKIES FOLLOW THE EXIT. The jar is keyed to the current egress identity
 *     (lib/proxy.js `identityTag()`), and is emptied when that identity changes,
 *     so a clearance cookie can never be replayed from the exit that rejected
 *     it. An exit that answers 401/403/429 is remembered as blocked (list mode
 *     skips it) and its cookies are dropped before the retry.
 *
...
const BLOCK_CODES = new Set([401, 403, 429]);   // the exit itself was rejected
...
/* Which egress identity the long-lived jar currently holds cookies for. */
let jarIdentity;

/*
 * The long-lived jar — but only while the exit is still the one those cookies
 * were handed out by. When the identity changes (a Tor NEWNYM rotation, a new
 * route) everything in there belongs to a different IP, and replaying a
 * clearance/bot cookie from the wrong IP is exactly what earns a 401. So the jar
 * is emptied together with the identity, and this returns null in the modes
 * where the exit changes on its own (list mode: per request · isolated Tor: per
 * connection) — there cookies live only for their own redirect chain.
 */
function jarFor(identity) {
  if (!identity) return null;
  if (jarIdentity !== identity) {
    cookieJar.clear();
...
/* Drop every cookie held for a host (and for its subdomains / parent domains). */
function forgetHost(host) {
  const h = normalizeHost(host);
  if (!h) return;
  for (const key of [...cookieJar.keys()]) {
    if (key === h || key.endsWith('.' + h) || h.endsWith('.' + key)) cookieJar.delete(key);
  }
}
...
async function runChain({ target, method, headers, body, timeoutMs, insecure, maxRedirects, attempt }) {
  // Read the identity ONCE: every hop of this chain keeps the cookie state it
  // started with, even if a NEWNYM rotation lands mid-chain.
  const jar = jarFor(proxyPool.identityTag());
  const chainJar = jar ? null : new Map();
  const writes = jar ? [jar] : [chainJar];
  const session = proxyPool.beginSession();
  const routeKey = session ? session.key : null;
  const done = (res) => ({ res, routeKey });
  ...
  for (let hop = 0; ; hop++) {
    const reqHeaders = { ...h };
    const cookie = cookiesFor(cur, chainJar, !!jar);
    ...
    if (!maxRedirects || !REDIRECT_CODES.has(res.status) || !res.headers.location) return done(res);
    const next = new URL(res.headers.location, cur);
    if (next.protocol !== 'http:' && next.protocol !== 'https:') return done(res);
    if (hop >= maxRedirects) return done(res);
...
async function fetchWithTimeout(urlStr, opts = {}, timeoutMs = 12000) {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    res = out.res;
    if (BLOCK_CODES.has(res.status)) {
      // The target rejected THIS exit. Remember it (list mode will skip it) and
      // drop the cookies it handed us: whatever a rejected exit told us must not
      // ride along into the retry on another exit.
      proxyPool.noteBlocked(out.routeKey, res.status === 429 ? 30000 : 60000);
      forgetHost(target.hostname);
    }
    // 401 = "this exit, not this request". Only worth another attempt when the
    // retry can actually land on a different exit.
    const retryable = RETRY_CODES.has(res.status) || (res.status === 401 && proxyPool.canRotate());
    if (attempt === attempts - 1 || !retryable) return res;
...
module.exports = { fetchWithTimeout, exitCheck, requestOnce, toHeaders, cookieJar, jarFor, forgetHost };
